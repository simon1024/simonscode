                ...
                <table id="table_dept_list" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr class="selected">
                            ...
                            <td><input type="text" name="dept_approver" value="{ename}" id="cur_approver_autocomplete"></td>
							<td class="td-actions">
...
<table id="add_dept_row" style="display:none">
    <tr class="selected dept_row">
		...
		<td><input type="text" name="dept_approver" placeholder="输入拼音进行搜索" id="cur_approver_autocomplete"></td>
        <td class="td-actions">
            <div class="hidden-phone visible-desktop btn-group">'
...
$(function() {
        ...
        $( "#cur_approver_autocomplete,#approver_autocomplete" ).autocomplete({
            source: availableTags,
            minChars:0
        });

    $('#add_dept_btn').click(function() {
        var rowHtml = $('#add_dept_row').html();
        rowHtml = rowHtml.replace("<tbody>", "");;
        rowHtml = rowHtml.replace("</tbody>", "");;
        rowHtml = rowHtml.replace("<TBODY>", "");;
        rowHtml = rowHtml.replace("</TBODY>", "");;
        $('#table_dept_list tbody').append(rowHtml);;
        var availableTags = [
        {employeeList}
                {label:"{username} {department} {position}", value:'{username}'},
        {/employeeList}
